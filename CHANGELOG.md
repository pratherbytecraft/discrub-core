...
- `DiscrubSetting.RETRY_WAIT` and its `AppSettings` field. The host reads it as the
  number of seconds to wait before the first retry of a failed request; each retry
  doubles it.
